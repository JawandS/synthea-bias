#!/usr/bin/env python3
"""Analyze COPD diagnosis probability with upstream features."""
...
def write_report(
    ...
    lines: List[str] = []
    lines.append("# COPD Diagnosis Probability Analysis")
    lines.append("")
    lines.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    lines.append(f"Dataset: `{csv_dir}`")
    lines.append("")
    lines.append("## Module summary (copd.json)")
    ...
    lines.append(f"- COPD prevalence: {format_percent(prevalence)}")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="COPD diagnosis probability analysis.")
    parser.add_argument(
        "--data",
    ...
    parser.add_argument(
        "--out",
        default=str(SCRIPT_DIR / "COPD_ANALYSIS.md"),
    ...
    copd_patients = load_condition_groups(conditions_path, {"copd": COPD_CODES})["copd"]
    ...
    labels = {patient_id: 1 if patient_id in copd_patients else 0 for patient_id in patient_ids}
    ...
    specs = [
        ...
        FeatureSpec(
            "Risk + asthma",
            ["age_years", "male", "income", "bmi", "smoker", "asthma"],
        ),
        FeatureSpec(
            "Risk + asthma + vitals",
            [
    ...
    module_summary = load_module_summary(
        Path(__file__).resolve().parents[2] / "src" / "main" / "resources" / "modules" / "copd.json"
    )
