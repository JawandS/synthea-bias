#!/usr/bin/env python3
"""Analyze sleep apnea diagnosis probability with upstream features."""
...
def load_module_summary(module_path: Path) -> Dict[str, List[str]]:
    summary = {
        "condition_codes": [],
        "procedure_codes": [],
        "observation_codes": [],
    ...
    for state in data.get("states", {}).values():
        ...
        if state_type == "SupplyList":
            for supply in state.get("supplies", []):
                code = supply.get("code") or {}
                if code.get("code"):
                    supply_codes.add(code["code"])
    ...
    summary["supply_codes"] = sorted(str(code) for code in supply_codes if code)
...
def write_report(
    ...
    lines: List[str] = []
    lines.append("# Sleep Apnea Diagnosis Probability Analysis")
    lines.append("")
    lines.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    lines.append(f"Dataset: `{csv_dir}`")
    lines.append("")
    lines.append("## Module summary (sleep_apnea.json)")
    ...
    if module_summary["supply_codes"]:
        lines.append(f"- Supply codes: {', '.join(module_summary['supply_codes'])}")
    lines.append("")
    lines.append("## Diagnosis prevalence")
    lines.append(f"- Sleep apnea prevalence: {format_percent(prevalence)}")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sleep apnea diagnosis probability analysis.")
    parser.add_argument(
        "--data",
    ...
    parser.add_argument(
        "--out",
        default=str(SCRIPT_DIR / "SLEEP_APNEA_ANALYSIS.md"),
    ...
    sleep_apnea_patients = load_condition_groups(
        conditions_path, {"sleep_apnea": SLEEP_APNEA_CODES}
    )["sleep_apnea"]
    ...
    labels = {
        patient_id: 1 if patient_id in sleep_apnea_patients else 0 for patient_id in patient_ids
    }
