...
Baseline dataset:
```bash
./run_synthea -p 25000 --exporter.csv.export=true --exporter.baseDirectory=./output_baseline Maine
```

Biased dataset (rural access bias override):
```bash
./run_synthea -p 25000 --exporter.csv.export=true --exporter.baseDirectory=./output_rural_bias --module_override=/home/js/contracts/synthea-bias/config/overrides_rural_sleep_apnea.properties Maine
```

## Run The Model
```bash
python3 scripts/sleep_apnea/main.py \
