#!/usr/bin/env python3
"""Analyze myocardial infarction diagnosis probability with upstream features."""
...
MI_CODES = {"22298006", "401303003", "401314000", "399211009"}
...
def load_module_summary(module_path: Path) -> Dict[str, List[str]]:
    ...
    summary["condition_codes"] = sorted(code for code in condition_codes if code)
    summary["procedure_codes"] = sorted(code for code in procedure_codes if code)
...
def write_report(
    ...
    lines: List[str] = []
    lines.append("# MI Diagnosis Probability Analysis")
    lines.append("")
    lines.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    lines.append(f"Dataset: `{csv_dir}`")
    lines.append("")
    lines.append("## Module summary (myocardial_infarction.json)")
    ...
    lines.append(f"- MI prevalence: {format_percent(prevalence)}")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="MI diagnosis probability analysis.")
    parser.add_argument(
        "--data",
    ...
    parser.add_argument(
        "--out",
        default=str(SCRIPT_DIR / "MI_ANALYSIS.md"),
    ...
    mi_patients = load_condition_groups(conditions_path, {"mi": MI_CODES})["mi"]
    ...
    labels = {patient_id: 1 if patient_id in mi_patients else 0 for patient_id in patient_ids}
    ...
    module_summary = load_module_summary(
        Path(__file__).resolve().parents[2] / "src" / "main" / "resources" / "modules" / "myocardial_infarction.json"
    )
